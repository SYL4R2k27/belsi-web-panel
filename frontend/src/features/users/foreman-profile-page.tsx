...
export default function ForemanProfilePage() {
  ...
  if (!profile) {
    return <div className="text-center text-muted-foreground">Бригадир не найден</div>
  }
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
              <div className="flex flex-col items-center gap-1 rounded-lg border p-4">
                ...
                <span className="text-2xl font-bold">{profile.team_members?.length ?? 0}</span>
                <span className="text-xs text-muted-foreground">В команде</span>
              </div>
              <div className="flex flex-col items-center gap-1 rounded-lg border p-4">
      ...
      <Tabs defaultValue="team">
        ...
        <TabsContent value="team" className="mt-4">
          {!profile.team_members || profile.team_members.length === 0 ? (
            <p className="text-muted-foreground text-sm py-8 text-center">Нет монтажников в команде</p>
          ) : (
